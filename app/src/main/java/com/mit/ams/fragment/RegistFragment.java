...
public class RegistFragment extends Fragment {
    ...
    private String TAG = RegistFragment.class.getSimpleName();
    ...
    private Button registBtn, getValiBtn;
    private EditText etTel, etVali, etPwd, etRePwd;
    private TextView loginNowTv;

    private String tel, vali, pwd, rePwd, valiNum = "0";
    ...
    private void initView(View view) {
        ...
        //页面布局初始化
        registBtn = (Button) view.findViewById(R.id.login_btn);
        ...
        etRePwd = (EditText) view.findViewById(R.id.re_pwd);
        loginNowTv = (TextView) view.findViewById(R.id.login_now);
        //添加监听
        registBtn.setOnClickListener(listener);
        getValiBtn.setOnClickListener(listener);
        loginNowTv.setOnClickListener(listener);
    }
    ...
    private void regist() {
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                try {
                    final String[] key = {"mobile", "pwd", "realName"};
                    final String[] value = {tel, pwd, tel};
                    String rs = WSClient.soapGetInfo("userRegister", key, value);
                    if (StringUtils.isEmpty(rs)) {
                        bundle.putString(BUNDLE_LOCAL_ERROR, "注册出错");
                    } else {
                        JSONObject resultObj = JSONObject.parseObject(rs);
                        String status = resultObj.getString("status");
                        if ("1".equals(status)) {
                            bundle.putString(BUNDLE_LOGIN_JUMP, "注册成功");
                        } else {
